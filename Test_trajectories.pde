...
void setup() {
  ...
  testMesh = rect_mesh(Xgrid, Ygrid, 1.0, 1.0, -.5, -.5); 
  ...
  testOrbit = new Orbit(.5, -.65, 10, c); 
  testOrbitA = new Orbit(.5, .65, 10, c); 
  
  testOrbitB = new Orbit(-.5, -.65, 10, c); 
  testOrbitC = new Orbit(-.5, .65, 10, c); 
  
  testOrbit2 = new Orbit(.5, .5, 10, c); 
  pointArray2[0][0] = 0.0; 
  pointArray2[0][1] = 0.0; 
...
float[] iteratePoint2(float[] thePoint) { 
...
void mousePressed() {
  pointArray2[0][0] = (mouseX/(gridW))*xrange - (xrange/2.0); 
  pointArray2[0][1] = (yrange/2.0)-(mouseY/(gridW))*yrange; 
  
  testOrbit2.setStart((mouseX/(gridW))*xrange - (xrange/2.0), (yrange/2.0)-(mouseY/(gridW))*yrange); 
  
  ptb[0] = (mouseX/(gridW))*xrange - (xrange/2.0); 
  ptb[1] = (yrange/2.0)-(mouseY/(gridW))*yrange; 
...
class Orbit {
  ...
  Orbit (float x, float y, int p, color theColor) {
    startX = x; 
    startY = y; 
    period = p; 
    numIterations = 0; 
...
float[][] rect_mesh(int numX, int numY, float W, float H, float BLx, float BLy) {
  int numPoints = (numX*numY); 
  float[][] the_mesh = new float[numPoints][2]; 
  float spacingX = W/float(numX); 
  float spacingY = H/float(numY); 
  
   for (int i = 0; i < numPoints; i++) {
   the_mesh[i][0] =  BLx + (i%numX)*spacingX +.5*spacingX; //x coordinate of point in mesh (as a real number)
    the_mesh[i][1] = BLy + (i/numX)*spacingY +.5*spacingY; //y coordinate of point in mesh (as a real number)
    }
...
float[] pointAvg(float[][] points) {
  ...
  x = x/float(points.length); 
  y = y/float(points.length);
